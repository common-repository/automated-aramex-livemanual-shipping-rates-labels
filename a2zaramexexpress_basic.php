	...
	if( !class_exists('a2z_aramexexpress_parent') ){
		Class a2z_aramexexpress_parent
		{
			...
			function my_label_page_contents(){
				...
				$url = site_url();
				if (isset($general_settings['a2z_aramexexpress_integration_key']) && !empty($general_settings['a2z_aramexexpress_integration_key'])) {
					echo "<iframe style='width: 100%;height: 100vh;' src='https://app.myshipi.com/embed/label.php?shop=".$url."&key=".$general_settings['a2z_aramexexpress_integration_key']."&show=ship'></iframe>";
				}
            }
			...
			public function hit_product_data_tab( $tabs) {
				...
				$tabs['hits_aramex_product_options'] = array(
					'label'		=> __( 'Shipi - ARAMEX Options', 'a2z_aramexexpress' ),
			...
			public function hit_bulk_order_menu( $actions ) {
				...
				$actions['create_label_shipo_aramex'] = __( 'Create Labels - Shipi [ARAMEX]', 'a2z_aramexexpress' );
				return $actions;
			}
			...
			public function hit_define_aramex_credentails( $user ){
				...
				$countires =  array(
				 ...
				 echo '<hr><h3 class="heading">ARAMEX Express - <a href="https://myshipi.com/" target="_blank">Shipi</a></h3>';
				    ?>
			...
			public function a2z_aramexexpress_plugin_action_links($links)
			{
				...
				$plugin_links = array(
					...
					'<a href="https://app.myshipi.com/support" target="_blank" >' . __('Support', 'a2z_aramexexpress') . '</a>'
					);
						...
						$output = (is_array($response) && isset($response['body'])) ? json_decode($response['body'],true) : [];
							if($output){
								...
								if(isset($output['status']) || isset($output['pickup_status'])){
									...
									update_option('hit_aramex_status_'.$order_id, 'Site not Connected with Shipi. Contact Shipi Team.');
								}
							}else{
								update_option('hit_aramex_status_'.$order_id, 'Site not Connected with Shipi. Contact Shipi Team.');
							}
